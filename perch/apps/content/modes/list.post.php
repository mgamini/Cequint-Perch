                    ...
                    if ($CurrentUser->userRole() == 'Admin' || ($CurrentUser->userRole() == 'Editor' && $item->get_option('adminOnly')==false)) {
                        ...
                        if ($level > $prev_level && ($level-$prev_level)>1) {
                            ...
                            for($i=0; $i<$diff; $i++) {
                                ...
                                echo '<td class="level'.($level-$diff+$i).' page inactive" colspan="4"><span>' . PerchUtil::html(PerchUtil::filename($path_parts[$i], false)) . '</span></td>';
                                echo '</tr>';
                            }
                        }
                        ...
                        echo '<tr class="'.PerchUtil::flip('odd').'">';
                            ...
                            echo '<td><a href="'.PerchUtil::html(PERCH_LOGINPATH).'/apps/content/edit/?id=' . PerchUtil::html($item->id()) . '" class="edit">' . PerchUtil::html($item->contentKey()) . '</a>';
                            ...
                            if ($CurrentUser->userRole() == 'Admin' || ($CurrentUser->userRole() == 'Editor' && $Settings->get('editorMayDeleteRegions')->settingValue())) {
                                echo '<a href="'.PerchUtil::html(PERCH_LOGINPATH).'/apps/content/delete/?id=' . PerchUtil::html($item->id()) . '" class="delete">'.PerchLang::get('Delete').'</a>';
                            }else{
                                echo '&nbsp;';
                            }
                            echo '</td>';
